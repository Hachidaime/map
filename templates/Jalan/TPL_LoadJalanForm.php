...
function Detail(){
    ...
    if(id != '' || id != null){
        ...
        data['id'] = id;
        {/literal}
        $.post(GoAjax({$system_id}, {$module_id}, {$action_id}, 'JalanDetail'), data, function(reply){
            $.each(reply, function(i, v){
                if(i == 'video_date' || i == 'survei_date')
                    if(v != null)
...
function Submit(){
    ...
    {/literal}
    $.post(GoAjax({$system_id}, {$module_id}, {$action_id}, 'SubmitJalan'), data, function(reply){
    {literal}
        $('#msg').html(reply.msg);
        GoTop();
		if(reply.id > 0){
