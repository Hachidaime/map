...
<div class="w3-row">
    <div class="w3-col l12 m12 s12 w3-padding-small">
        <button type="button" class="w3-button w3-hover-white w3-hover-text-theme-d2 w3-theme-d2 w3-text-white w3-border-theme-d2 w3-border w3-ripple w3-round-xxlarge w3-padding-small w3-margin-bottom" onclick="GoMenu({$system_id}, {$module_id}, {$action_id}, 'LoadActionForm');" style="width: 150px;" >Add Action</button>
    </div>
</div>
...
function Search(page = "1"){
    ...
    {/literal}
    $('#data-content').load(GoAjax('{$system_id}','{$module_id}','{$action_id}','LoadActionSearch')+'&page='+page+'&'+search);
    {literal}
}
...
function Change(id){
    ...
    {/literal}
    GoMenu('{$system_id}','{$module_id}','{$action_id}','LoadActionForm');
    {literal}
}
...
function Delete(params){
	var data = "id="+params;
    {/literal}
	var url = GoAjax('{$system_id}','{$module_id}','{$action_id}','DeleteAction');
    {literal}
